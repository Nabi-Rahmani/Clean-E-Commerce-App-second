...
import 'package:e_clean_fcm/features/cart/widgets/cart_design.dart';
...
class MyCartItems extends ConsumerWidget {
  MyCartItems({super.key});
  final _fireStore = FirebaseFirestore.instance;
  void _deleteMessage(BuildContext context, String documentId) async {
    try {
      await _fireStore.collection('CartItems').doc(documentId).delete();
      if (Platform.isIOS) {
        showCupertinoDialog(
          // ignore: use_build_context_synchronously
          context: context,
          builder: (BuildContext context) => CupertinoAlertDialog(
            title: const Text('Delete the Cart'),
            content: const Text('The Cart has been deleted successfully.'),
            actions: <Widget>[
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData) {
          for (var doc in snapshot.data!.docs) {
            ...
            final categoryGridView = ProductCard(
